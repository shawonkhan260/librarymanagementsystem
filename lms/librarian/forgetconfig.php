...
if(isset($_POST[submittoken]))
{
	...
	$query="select * from librarian_reg where token='$token' and tokenExpire > NOW()";
...
if(isset($_POST[submitpassword]))
{
	...
	$query="update librarian_reg SET password='$password', token='',tokenExpire='' where token='$token'";
	mysqli_query($connect,$query) or die ('Unable to execute query. '. mysqli_error($link));
	header('location:llogin.php?password=');
